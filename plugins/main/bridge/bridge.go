...
func validateCniBrInterface(intf current.Interface, n *NetConf) (cniBridgeIf, error) {
	...
	brFound := cniBridgeIf{found: false}
	if intf.Name == "" {
		return brFound, fmt.Errorf("Bridge interface name %s missing in prevResult", n.BrName)
	}
	link, err := netlink.LinkByName(intf.Name)
	if err != nil {
		return brFound, fmt.Errorf("Interface name %s for bridge in prevResult not found", intf.Name)
	}
...
func validateCniVethInterface(intf *current.Interface, brIf cniBridgeIf, contIf cniBridgeIf) (cniBridgeIf, error) {
	...
	var err error
	vethFound := cniBridgeIf{found: false}

	if intf.Name == "" {
		return vethFound, fmt.Errorf("Veth interface name missing in prevResult: %v", intf.Name)
	}

	if intf.Sandbox != "" {
		// skip, we're looking for veth in host namespace
		return vethFound, nil
	}

	link, err = netlink.LinkByName(intf.Name)
	if err != nil {
		return vethFound, fmt.Errorf("Container Interface name in prevResult: %s not found", intf.Name)
	}
...
func validateCniContainerInterface(intf current.Interface) (cniBridgeIf, error) {
	...
	vethFound := cniBridgeIf{found: false}
	if intf.Name == "" {
		return vethFound, fmt.Errorf("Container interface name missing in prevResult: %v", intf.Name)
	}
	link, err = netlink.LinkByName(intf.Name)
	if err != nil {
		return vethFound, fmt.Errorf("Container Interface name in prevResult: %s not found", intf.Name)
	...
	if intf.Sandbox == "" {
		return vethFound, fmt.Errorf("Error: Container interface %s should not be in host namespace", link.Attrs().Name)
	}
...
func cmdCheck(args *skel.CmdArgs) error {
	...
	// TODO - Is there another check that can be done?
	//handle := netns.Get()
	//if strings.Compare(handle.UniqueId(), contMap.Sandbox) != 0 {
	//	  return fmt.Errorf("Sandbox in prevResult: %s doesn't match current netns: %s",
	//	  	 		     args.Netns, contMap.Sandbox, args.Netns)
	//}
	...
	if err := netns.Do(func(_ ns.NetNS) error {
		err = utils.CheckPrevResultIPs(args.IfName, result.IPs)
		if err != nil {
			return err
		}

		err = utils.CheckPrevResultRoute(result.Routes)
		if err != nil {
			return err
		}
		return nil
