...
func createMacvlan(conf *NetConf, ifName string, netns ns.NetNS) (*current.Interface, error) {
	...
	err = netns.Do(func(_ ns.NetNS) error {
		...
		// TODO: Bug?  macvlan's ParentIndex isn't set
		/*
			if contMacvlan.Attrs().ParentIndex != m.Attrs().Index {
			   return fmt.Errorf("failed to set macvlan %q Parent Index is: %v should be %v", ifName, contMacvlan.Attrs().ParentIndex, m.Attrs().Index)
			}
		*/
...
func validateCniContainerInterface(intf current.Interface, parentIndex int, modeExpected string) error {
	...
	//if parentIndex != macv.Attrs().ParentIndex {
	//	return fmt.Errorf("Container Macvlan Master %d does not match expected value: %d", macv.Attrs().ParentIndex, parentIndex)
	//}
