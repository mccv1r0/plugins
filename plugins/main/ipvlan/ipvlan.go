...
func createIpvlan(conf *NetConf, ifName string, netns ns.NetNS) (*current.Interface, error) {
	...
	err = netns.Do(func(_ ns.NetNS) error {
		...
		// TODO: Bug?  ipvlan's ParentIndex isn't set
		//if contIpvlan.Attrs().ParentIndex != m.Attrs().Index {
		//   return fmt.Errorf("failed to set ipvlan %q Parent Index is: %v should be %v", ifName, contIpvlan.Attrs().ParentIndex, m.Attrs().Index)
		//}
		return nil
	})
	if err != nil {
	...
	for _, intf := range result.Interfaces {
		if args.IfName == intf.Name {
			if args.Netns == intf.Sandbox {
				//contMap.Name = intf.Name
				//contMap.Mac = intf.Mac
				//contMap.Sandbox = intf.Sandbox
				contMap = *intf
				continue
			}
		}
