...
func buildOneConfig(name, cniVersion string, master string, orig *Net, prevResult types.Result) (*Net, error) {
	var err error
...
func ipvlanAddCheckDelTest(conf, IFNAME string, originalNS ns.NetNS) {
	targetNs, err := testutils.NewNS()
	Expect(err).NotTo(HaveOccurred())
	defer targetNs.Close()
	...
	cniVersion := "0.4.0"
	newConf, err := buildOneConfig("testConfig", cniVersion, MASTER_NAME, n, result)
	Expect(err).NotTo(HaveOccurred())
...
var _ = Describe("ipvlan Operations", func() {
    ...
    "ipam": {
		...
		ipvlanAddCheckDelTest(conf, IFNAME, originalNS)
	})
